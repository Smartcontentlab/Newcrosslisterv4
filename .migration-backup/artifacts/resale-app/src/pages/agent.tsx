  ...
  return (
    <div className="space-y-2 mt-4 group">
      <div className="flex items-center justify-between">
        <span className="font-pixel text-[10px] text-muted-foreground uppercase tracking-widest">{label}</span>
        <button
          onClick={copy}
      ...
      <pre
        className="text-xs font-mono rounded-lg p-4 overflow-x-auto shadow-inner"
  ...
  return (
    <div className={`glass-card rounded-xl overflow-hidden border ${open ? colors[glowColor] : 'border-border/50'} transition-colors duration-300`}>
      <button
        ...
        className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-all bg-black/20"
      >
      ...
      {open && <div className="p-5 border-t border-white/5 animate-in slide-in-from-top-2 duration-200">{children}</div>}
    </div>
  );
}
...
# Then call: ${apiUrl}/{tool_name_as_path} with the arguments`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto">
      <div className="flex items-end justify-between border-b border-border/50 pb-4">
        <div>
          <h1 className="font-pixel text-xl tracking-wide uppercase flex items-center gap-3">
            <Bot className="text-primary" /> Agent Hub
          </h1>
          <p className="text-xs font-sans text-muted-foreground uppercase tracking-widest mt-2">API ACCESS FOR AUTONOMOUS AUTOMATION</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Items', value: status?.stats?.totalItems ?? '—', color: 'text-primary text-glow-pink' },
          { label: 'Pending Posts', value: status?.stats?.pendingPosts ?? '—', color: 'text-yellow-400' },
          { label: 'Total Listings', value: status?.stats?.totalListings ?? '—', color: 'text-accent' },
        ].map(s => (
          <div key={s.label} className="glass-card rounded-xl p-5 text-center border border-border/50 shadow-sm bg-black/20">
      ...
      <div className="glass-card rounded-xl p-6 border-primary/30 shadow-[0_0_30px_rgba(255,45,120,0.05)] relative overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
        <h2 className="font-pixel text-xs text-primary uppercase tracking-widest mb-4">Base Endpoint</h2>
        <div className="flex items-center gap-3 relative z-10">
          <code className="flex-1 font-mono text-sm bg-black/60 text-foreground px-4 py-3 rounded-lg border border-white/5 select-all shadow-inner">
            {apiUrl || 'loading...'}
          </code>
          <button
            onClick={copyEndpoint}
            className="p-3 rounded-lg border border-primary/30 text-primary hover:bg-primary/10 hover:shadow-[0_0_15px_rgba(255,45,120,0.2)] transition-all"
          >
            <Copy size={16} />
          </button>
        </div>
          ...
          {!queue || queue.count === 0 ? (
            ...
            <div className="space-y-3">
              {queue.queue?.slice(0, 8).map((item: any) => (
                <div key={item.itemId} className="flex items-center justify-between p-4 rounded-lg bg-black/30 border border-white/5 hover:border-primary/20 transition-colors">
                  <div>
                    <p className="font-sans font-bold text-sm text-foreground">{item.title}</p>
                    <p className="font-pixel text-[8px] text-muted-foreground mt-1.5 uppercase">ID #{item.itemId} · {item.brand || 'No brand'}</p>
                  </div>
                  <div className="flex gap-1.5 flex-wrap justify-end max-w-[200px]">
                    {item.pendingMarketplaces.map((mp: string) => (
                      <span key={mp} className="px-2 py-1 rounded bg-primary/10 border border-primary/20 text-primary text-[9px] font-bold uppercase tracking-wider">
                        {mp}
                      </span>
                    ))}
                  </div>
        ...
        <Section title="Raw API Reference" icon={<Globe size={16} />} glowColor="primary">
          <div className="space-y-4">
            {[
              ...
              <div key={ep.p} className="rounded-lg p-4 bg-black/30 border border-white/5">
                <div className="flex items-center gap-3 mb-2">
                  <span className={`font-pixel text-[10px] px-2 py-1 rounded ${ep.m === 'GET' ? 'bg-accent/20 text-accent border border-accent/20' : 'bg-primary/20 text-primary border border-primary/20'}`}>
                    {ep.m}
                  </span>
                  <code className="font-mono text-sm text-foreground">{ep.p}</code>
                </div>
                <p className="font-sans text-sm text-muted-foreground">{ep.d}</p>
                {ep.body && <pre className="mt-3 text-xs font-mono text-muted-foreground bg-black/50 p-3 rounded border border-white/5">{ep.body}</pre>}
              </div>
            ))}
          </div>
