...
export default function ListingStudio() {
  ...
  return (
    <div className="space-y-6 pb-28">
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-card px-4 py-3  lg:left-[248px] lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span className={`h-2 w-2 rounded-full ${itemId ? 'bg-success' : 'bg-primary'}`} />{itemId ? 'Changes save to your canonical listing.' : 'Save this item before preparing marketplace drafts.'}</div><div className="grid grid-cols-3 gap-2 sm:flex"><button type="button" onClick={saveAndContinue} disabled={isSaving} className="inline-flex items-center justify-center gap-2 rounded-none bg-primary border-2 border-foreground hover:bg-accent-hover px-3 py-2.5 text-xs font-extrabold uppercase tracking-[0.05em] text-primary-foreground  disabled:opacity-50">{isSaving ? <Loader2 className="animate-spin" size={15} /> : <Save size={15} />}{isSaving ? 'Saving' : 'Save & Continue'}</button><button type="button" onClick={() => requestAiAssist('all')} disabled={isAssisting} className="inline-flex items-center justify-center gap-2 rounded-none border border-border bg-success-tint px-3 py-2.5 text-xs font-extrabold uppercase tracking-[0.05em] text-foreground hover:bg-success-tint disabled:opacity-50"><Sparkles size={15} />AI assist</button><button type="button" onClick={generateMarketplaceDrafts} disabled={isGeneratingDrafts} className="inline-flex items-center justify-center gap-2 rounded-none border border-border bg-card px-3 py-2.5 text-xs font-extrabold uppercase tracking-[0.05em] text-foreground hover:border-border disabled:opacity-50">{isGeneratingDrafts ? <Loader2 className="animate-spin" size={15} /> : <ArrowRight size={15} />}{isGeneratingDrafts ? 'Preparing' : 'Generate P/D/M'}</button></div></div></div>
    </div>
  );
}
