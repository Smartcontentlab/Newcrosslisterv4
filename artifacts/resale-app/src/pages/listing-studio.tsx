...
export default function ListingStudio() {
  ...
  return (
    <div className="space-y-6 pb-28">
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border/80 bg-background/95 px-4 py-3 backdrop-blur-xl lg:left-72 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span className={`h-2 w-2 rounded-full ${itemId ? 'bg-accent' : 'bg-primary'}`} />{itemId ? 'Changes save to your canonical listing.' : 'Save this item before preparing marketplace drafts.'}</div><div className="grid grid-cols-3 gap-2 sm:flex"><button type="button" onClick={saveAndContinue} disabled={isSaving} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2.5 text-xs font-bold text-primary-foreground hover:brightness-110 disabled:opacity-50">{isSaving ? <Loader2 className="animate-spin" size={15} /> : <Save size={15} />}{isSaving ? 'Saving' : 'Save & Continue'}</button><button type="button" onClick={() => requestAiAssist('all')} disabled={isAssisting} className="inline-flex items-center justify-center gap-2 rounded-lg border border-accent/40 bg-accent/10 px-3 py-2.5 text-xs font-bold text-accent hover:bg-accent/15 disabled:opacity-50"><Sparkles size={15} />AI assist</button><button type="button" onClick={generateMarketplaceDrafts} disabled={isGeneratingDrafts} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-background/55 px-3 py-2.5 text-xs font-bold text-foreground hover:border-primary/40 disabled:opacity-50">{isGeneratingDrafts ? <Loader2 className="animate-spin" size={15} /> : <ArrowRight size={15} />}{isGeneratingDrafts ? 'Preparing' : 'Generate P/D/M'}</button></div></div></div>
    </div>
  );
}
