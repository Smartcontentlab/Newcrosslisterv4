...
export default function ListingStudio() {
  ...
  const [aiFocus, setAiFocus] = useState<'all' | 'title' | 'tags' | null>(null);
  ...
  const requestAiAssist = async (focus: 'all' | 'title' | 'tags' = 'all') => {
    setAiNotice(null);
    if (!form.title.trim()) {
      setAiNotice({ tone: 'error', text: 'Add an item title in section 03 first. The co-pilot works only from the facts you enter.' });
      return;
    }
    setIsAssisting(true);
    setAiFocus(focus);
    try {
      const response = await apiFetch('/api/workflow/ai-assist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          focus, title: form.title.trim(), description: form.description || undefined, brand: form.brand || undefined, model: form.model || undefined,
          category: form.category || undefined, size: form.size || undefined, color: form.color || undefined, measurements: form.measurements || undefined,
          material: marketplaceDetails.material || undefined, flaws: marketplaceDetails.flaws || undefined, condition: form.condition,
      ...
      const suggestion: AiSuggestion = {
        ...
        tags: focus === 'title' ? form.tags : Array.isArray(data.tags) ? data.tags.join(', ') : form.tags,
      ...
      setAiNotice({ tone: 'info', text: data.usedFallback ? 'The AI model did not answer, so this is a template built from your entries. Edit it below, then apply.' : 'Suggestion ready below. Edit it if you like, then apply.' });
    } catch (error) {
      setAiNotice({ tone: 'error', text: `${error instanceof Error ? error.message : 'AI assistance is unavailable.'} You can keep editing by hand.` });
    } finally { setIsAssisting(false); setAiFocus(null); }
  };
  ...
  const estimatePrice = () => {
    // Same "AI suggestion?" flow as the price box: you choose whether to add it.
    setAiNotice(null);
    if (!form.title.trim()) { setAiNotice({ tone: 'error', text: 'Add an item title in section 03 first so the AI knows what to price.' }); return; }
    setPriceAsk((count) => count + 1);
    document.getElementById('pricing-panel')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  };
    ...
    <div className="space-y-6 pb-28">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.06fr_0.94fr]">
        ...
        <aside className="cx-panel rounded-none p-5 sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => requestAiAssist('all')} disabled={isAssisting} className="rounded-none border border-border bg-success-tint px-3 py-2 text-left text-xs font-semibold text-foreground hover:bg-success-tint disabled:opacity-50">{aiFocus === 'all' ? <Loader2 size={14} className="mb-1 animate-spin" /> : <Sparkles size={14} className="mb-1" />}Write description</button><button type="button" onClick={() => requestAiAssist('title')} disabled={isAssisting} className="rounded-none border border-border bg-card px-3 py-2 text-left text-xs font-semibold hover:border-border disabled:opacity-50">{aiFocus === 'title' ? <Loader2 size={14} className="mb-1 animate-spin" /> : <Wand2 size={14} className="mb-1 text-foreground" />}Improve title</button><button type="button" onClick={() => requestAiAssist('tags')} disabled={isAssisting} className="rounded-none border border-border bg-card px-3 py-2 text-left text-xs font-semibold hover:border-border disabled:opacity-50">{aiFocus === 'tags' ? <Loader2 size={14} className="mb-1 animate-spin" /> : <Tags size={14} className="mb-1 text-foreground" />}Suggest tags</button><button type="button" onClick={estimatePrice} className="rounded-none border border-border bg-card px-3 py-2 text-left text-xs font-semibold hover:border-border disabled:opacity-50"><CircleDollarSign size={14} className="mb-1 text-foreground" />Estimate price</button></div>
          {aiNotice && <p role={aiNotice.tone === 'error' ? 'alert' : 'status'} className={`mt-4 border-2 border-border px-3 py-2 text-xs font-semibold text-foreground ${aiNotice.tone === 'error' ? 'border-l-[10px] border-l-destructive bg-warning-tint' : 'bg-success-tint'}`}>{aiNotice.text}</p>}
