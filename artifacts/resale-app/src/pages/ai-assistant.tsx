  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto">
      <div className="flex items-end justify-between border-b border-border/50 pb-4">
        <div>
          <h1 className="font-pixel text-xl tracking-wide uppercase flex items-center gap-3">
            <Sparkles className="text-secondary" /> AI Copilot
          </h1>
          <p className="text-xs font-sans text-muted-foreground uppercase tracking-widest mt-2">PRICING, TRENDS & AUTOMATION</p>
        </div>
      </div>

      <Tabs defaultValue="chat" className="w-full">
        <TabsList className="glass-card border border-border/50 p-1 bg-black/20 mb-6 flex space-x-1 rounded-xl h-auto">
          <TabsTrigger value="chat" className="flex-1 font-sans font-bold py-3 data-[state=active]:bg-primary/20 data-[state=active]:text-primary rounded-lg transition-all text-muted-foreground data-[state=active]:shadow-[0_0_15px_rgba(255,45,120,0.2)]" data-testid="tab-chat">
            <Sparkles size={16} className="mr-2" /> Chat
          </TabsTrigger>
          <TabsTrigger value="price" className="flex-1 font-sans font-bold py-3 data-[state=active]:bg-accent/20 data-[state=active]:text-accent rounded-lg transition-all text-muted-foreground data-[state=active]:shadow-[0_0_15px_rgba(0,255,209,0.2)]" data-testid="tab-price">
            <DollarSign size={16} className="mr-2" /> Estimator
          </TabsTrigger>
          <TabsTrigger value="listing" className="flex-1 font-sans font-bold py-3 data-[state=active]:bg-secondary/20 data-[state=active]:text-secondary rounded-lg transition-all text-muted-foreground data-[state=active]:shadow-[0_0_15px_rgba(192,132,252,0.2)]" data-testid="tab-listing">
        ...
        <TabsContent value="chat" className="h-[600px] flex flex-col">
          <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2">
            {history.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                <Sparkles size={40} className="text-secondary mb-4 animate-glow-pulse-purple" />
                <p className="font-pixel text-xs text-muted-foreground uppercase">SYSTEM READY</p>
              </div>
            )}
            {history.map((h, i) => (
              <div key={i} className={`flex ${h.role === 'u' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 rounded-xl font-sans text-sm leading-relaxed ${
                  h.role === 'u' ? 'bg-primary/20 border border-primary/30 text-primary-foreground ml-12 shadow-[0_0_15px_rgba(255,45,120,0.1)]' : 'glass-card border-border/50 mr-12 text-foreground'
                }`}>
                  {h.text}
                </div>
              </div>
          ...
          <form onSubmit={send} className="shrink-0 relative">
            <Input 
              ...
              className="bg-black/40 border-white/10 h-14 pl-5 pr-14 font-sans text-base focus-visible:ring-secondary focus-visible:border-secondary shadow-inner" 
              disabled={chat.isPending}
            />
            <button 
              type="submit" 
              disabled={chat.isPending || !msg.trim()}
              className="absolute right-2 top-2 bottom-2 w-10 flex items-center justify-center rounded-lg bg-secondary/10 text-secondary hover:bg-secondary/20 transition-colors disabled:opacity-50"
            >
              <Send size={16} />
            </button>
          </form>
        ...
        <TabsContent value="price" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-card rounded-xl p-6 border-border/50 border-t-accent/30 shadow-[0_-2px_20px_rgba(0,255,209,0.05)]">
            <h2 className="font-pixel text-xs text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2">
              <DollarSign size={14} className="text-accent" /> Item Details
            </h2>
            <form onSubmit={handlePriceEstimate} className="space-y-4">
              <div>
                <Label>Title</Label>
                <Input value={priceForm.title} onChange={e => setPriceForm({ ...priceForm, title: e.target.value })} required className="bg-black/20 focus:border-accent" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Brand</Label>
                  <Input value={priceForm.brand} onChange={e => setPriceForm({ ...priceForm, brand: e.target.value })} className="bg-black/20 focus:border-accent" />
                </div>
                <div>
                  <Label>Model</Label>
                  <Input value={priceForm.model} onChange={e => setPriceForm({ ...priceForm, model: e.target.value })} className="bg-black/20 focus:border-accent" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Condition</Label>
                  <Input value={priceForm.condition} onChange={e => setPriceForm({ ...priceForm, condition: e.target.value })} className="bg-black/20 focus:border-accent" />
                </div>
                <div>
                  <Label>Category</Label>
                  <Input value={priceForm.category} onChange={e => setPriceForm({ ...priceForm, category: e.target.value })} className="bg-black/20 focus:border-accent" />
                </div>
              </div>
              <button type="submit" disabled={getPriceEstimate.isPending} className="w-full py-3 bg-accent text-accent-foreground font-sans font-bold rounded-lg disabled:opacity-50 hover:bg-accent/90 transition-colors shadow-[0_0_15px_rgba(0,255,209,0.2)] mt-4">
          ...
          <div className="glass-card rounded-xl p-6 border-border/50 flex flex-col">
            <h2 className="font-pixel text-xs text-muted-foreground uppercase tracking-widest mb-6">Prediction</h2>
            {priceResult ? (
              <div className="space-y-6 flex-1">
                <div className="text-center p-8 bg-black/40 rounded-xl border border-accent/20 shadow-[inset_0_0_30px_rgba(0,255,209,0.05)]">
                  <p className="text-xs font-sans text-muted-foreground uppercase tracking-widest mb-3">Optimal Target</p>
                  <p className="font-pixel text-5xl text-accent text-glow-mint">${priceResult.suggestedPrice.toFixed(2)}</p>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-black/20 p-4 rounded-lg border border-white/5 text-center">
                    <p className="text-[10px] font-pixel text-muted-foreground uppercase mb-2">Min</p>
                    <p className="font-sans font-bold text-foreground text-lg">${priceResult.minPrice.toFixed(2)}</p>
                  </div>
                  <div className="bg-black/20 p-4 rounded-lg border border-white/5 text-center">
                    <p className="text-[10px] font-pixel text-muted-foreground uppercase mb-2">Max</p>
                    <p className="font-sans font-bold text-foreground text-lg">${priceResult.maxPrice.toFixed(2)}</p>
                  </div>
                  <div className="bg-black/20 p-4 rounded-lg border border-white/5 text-center">
                    <p className="text-[10px] font-pixel text-muted-foreground uppercase mb-2">Confidence</p>
                    <p className={`font-sans font-bold text-sm mt-1 uppercase ${
                      priceResult.confidence === 'high' ? 'text-accent' : 
                      priceResult.confidence === 'medium' ? 'text-secondary' : 'text-muted-foreground'
                    }`}>
                ...
                {priceResult.reasoning && (
                  <div className="bg-black/20 p-4 rounded-lg border border-white/5 mt-auto">
                    <p className="text-[10px] font-pixel text-muted-foreground uppercase mb-2">Analysis</p>
        ...
        <TabsContent value="listing" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-card rounded-xl p-6 border-border/50 border-t-secondary/30 shadow-[0_-2px_20px_rgba(192,132,252,0.05)]">
            <h2 className="font-pixel text-xs text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2">
              <Wand2 size={14} className="text-secondary" /> Generate Copy
            </h2>
            <form onSubmit={handleGenerateListing} className="space-y-4">
              <div>
                ...
                <Select value={listingForm.itemId} onValueChange={v => setListingForm({ ...listingForm, itemId: v })}>
                  <SelectTrigger className="bg-black/20 focus:border-secondary"><SelectValue placeholder="Select item..." /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={listingForm.marketplace} onValueChange={v => setListingForm({ ...listingForm, marketplace: v as GenerateListingRequestMarketplace })}>
                  <SelectTrigger className="bg-black/20 focus:border-secondary"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-card border-border">
              ...
              <button type="submit" disabled={generateListing.isPending || !listingForm.itemId} className="w-full py-3 bg-secondary text-secondary-foreground font-sans font-bold rounded-lg disabled:opacity-50 hover:bg-secondary/90 transition-colors shadow-[0_0_15px_rgba(192,132,252,0.2)] mt-4">
          ...
          <div className="glass-card rounded-xl p-6 border-border/50 flex flex-col">
            <h2 className="font-pixel text-xs text-muted-foreground uppercase tracking-widest mb-6">Generated Content</h2>
            {listingResult ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center opacity-30 text-center">
                <Wand2 size={48} className="text-secondary mb-4" />
