...
export default function AuthPage() {
  ...
  const field = "h-11 w-full rounded-xl border border-input bg-card pl-10 pr-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/25";
    ...
    <main className="min-h-[100dvh] bg-background px-5 py-10 text-foreground sm:px-8">
      <section className="mx-auto grid min-h-[calc(100dvh-5rem)] max-w-5xl items-center gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="tile-inverse corner-ticks hidden p-10 lg:block">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-on-inverse text-inverse"><Sparkle size={20} /></span>
          <p className="mt-9 font-mono text-[0.6875rem] uppercase tracking-[0.08em] text-label-on-inverse">[ Private resale operations ]</p>
          ...
          <div className="mt-9 flex flex-wrap gap-2">
            {MARKETS.map((market) => (
              <span key={market.code} className="inline-flex items-center gap-2 rounded-full border border-dot-on-inverse px-3 py-1.5 font-mono text-[0.6875rem] uppercase tracking-[0.06em]">
        ...
        <div className="mx-auto w-full max-w-md rounded-[20px] border border-border bg-card p-6 sm:p-8">
          <div className="mb-8 flex items-center gap-2">
            ...
            <span className="font-display text-xl font-bold tracking-tight">CrossLinkOS</span>
          </div>
          ...
          <div className="mt-7 grid grid-cols-2 gap-1 rounded-full bg-muted p-1" role="tablist" aria-label="Sign in or create account">
            <button type="button" role="tab" aria-selected={mode === "signin"} onClick={() => { setMode("signin"); setError(null); setNotice(null); }} className={`h-10 rounded-full text-sm font-semibold transition ${mode === "signin" ? "bg-card text-foreground" : "text-ink-2 hover:text-foreground"}`}>Sign in</button>
            <button type="button" role="tab" aria-selected={mode === "signup"} onClick={() => { setMode("signup"); setError(null); setNotice(null); }} className={`h-10 rounded-full text-sm font-semibold transition ${mode === "signup" ? "bg-card text-foreground" : "text-ink-2 hover:text-foreground"}`}>Create account</button>
          </div>
          ...
          <form className="mt-6 space-y-4" onSubmit={submit}>
            ...
            {configurationError && <p role="alert" className="rounded-xl border border-destructive/40 bg-danger-tint p-3 text-xs leading-5 text-destructive">{configurationError}</p>}
            {error && <p role="alert" className="rounded-xl border border-destructive/40 bg-danger-tint p-3 text-xs leading-5 text-destructive">{error}</p>}
            {notice && <p role="status" className="flex gap-2 rounded-xl border border-success/40 bg-success-tint p-3 text-xs leading-5 text-success"><Check size={14} className="mt-0.5 shrink-0" />{notice}</p>}
            <button disabled={busy || Boolean(configurationError)} className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-foreground px-4 text-sm font-semibold text-background transition hover:bg-foreground/85 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          <p className="mt-6 text-center font-mono text-[0.6875rem] text-muted-foreground">Marketplace passwords are never stored here.</p>
        </div>
      </section>
    </main>
  );
