...
const stateMeta: Record<SessionState, { label: string; glyph: string; className: string; description: string }> = {
  ...
  signed_in: { label: 'Signed in locally', glyph: '●', className: 'bg-success-tint text-success', description: 'The extension saw an active local session in this Chrome profile. CrossLinkOS never receives the session credential.' },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={(
          <button type="button" onClick={checkExtension} className="inline-flex h-11 items-center gap-2 rounded-full border border-foreground px-5 text-sm font-medium transition-colors hover:bg-muted">
      ...
      {extension.detected ? (
        <section className="flex flex-col gap-4 rounded-[20px] border border-success/40 bg-success-tint p-5 sm:flex-row sm:items-center sm:justify-between" role="status">
          <div className="flex gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-success text-success-foreground"><CheckCircle2 size={20} /></span>
            <div>
              <p className="font-semibold text-success">Extension detected in this Chrome profile</p>
          ...
          <a href={EXTENSION_ZIP} download className="inline-flex h-10 items-center gap-2 self-start rounded-full border border-success px-4 text-sm font-medium text-success hover:bg-success/10 sm:self-auto"><Download size={15} /> Re-download</a>
        </section>
      ) : (
        <section className="tile-inverse corner-ticks flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-xl">
            <p className="font-mono text-[0.6875rem] uppercase tracking-[0.08em] text-label-on-inverse">[ Extension not detected ]</p>
          ...
          <a href={EXTENSION_ZIP} download className="inline-flex h-12 shrink-0 items-center gap-2 self-start rounded-full bg-accent-alt px-6 text-sm font-semibold text-inverse sm:self-auto">
      ...
      <section aria-label="Marketplace connections" className="grid gap-4 lg:grid-cols-3">
        {PLATFORMS.map((platform) => {
          ...
          return (
            <article key={platform.id} className="flex flex-col rounded-[20px] border border-border bg-card p-5">
              <div className="flex items-start justify-between gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-accent-tint font-mono text-sm font-medium text-accent-text">{platform.code}</span>
                <span className={`inline-flex h-[26px] items-center gap-1.5 rounded-full px-2.5 font-mono text-[0.6875rem] uppercase tracking-[0.06em] ${meta.className}`}><span aria-hidden="true">{meta.glyph}</span>{meta.label}</span>
              </div>
              <h2 className="mt-5 font-display text-xl font-bold">{platform.name}</h2>
              ...
              <div className="mt-4 rounded-xl border border-border p-3">
                ...
                {checkedAt && <p className="mt-2 font-mono text-[0.6875rem] text-muted-foreground">Checked {new Date(checkedAt).toLocaleString()}</p>}
              </div>
              <a href={platform.url} target="_blank" rel="noreferrer" className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-foreground text-sm font-medium text-background hover:bg-foreground/85">Open {platform.name} <ExternalLink size={14} /></a>
            </article>
          );
        })}
      </section>

      <section aria-label="Install steps" className="rounded-[20px] border border-border bg-card p-6">
        <div className="flex items-baseline justify-between">
          <h2 className="font-display text-xl font-bold">Install in six steps</h2>
          <button type="button" onClick={copyExtensionsUrl} className="inline-flex items-center gap-2 rounded-full border border-border px-3.5 py-1.5 font-mono text-xs hover:bg-muted"><Copy size={13} /> chrome://extensions</button>
        </div>
      ...
      <section aria-label="More marketplaces" className="rounded-[20px] border border-border bg-card px-7 py-6">
        <div className="flex items-baseline justify-between pb-3">
          <h2 className="font-display text-xl font-bold">More marketplaces</h2>
        ...
        {PLANNED.map((market) => (
          <div key={market.name} className="grid grid-cols-[36px_minmax(0,1fr)_auto] items-center gap-x-4 border-t border-border py-3">
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-dashed border-input font-mono text-[0.625rem] text-muted-foreground">{market.code}</span>
            <div className="min-w-0"><p className="text-[0.9375rem] font-medium">{market.name}</p><p className="font-mono text-xs text-muted-foreground">{market.method}</p></div>
            <span className="inline-flex h-[26px] items-center gap-1.5 rounded-full border border-dashed border-input px-2.5 font-mono text-[0.6875rem] uppercase tracking-[0.06em] text-muted-foreground"><span aria-hidden="true">○</span>Planned</span>
          </div>
        ))}
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3" aria-label="Safety">
        {[
          ...
          <div key={card.title} className="rounded-[20px] border border-border bg-card p-5">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-accent-tint text-accent-text">{card.icon}</span>
