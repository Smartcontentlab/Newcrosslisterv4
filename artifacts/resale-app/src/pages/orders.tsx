...
const lifecycleMeta: Record<OrderStatus, { label: string; className: string }> = {
  pending: { label: 'Sale recorded', className: 'border-primary/35 bg-primary/10 text-primary' },
  awaiting_shipment: { label: 'Ready for fulfillment', className: 'border-secondary/35 bg-secondary/10 text-secondary' },
  shipped: { label: 'Shipped', className: 'border-accent/35 bg-accent/10 text-accent' },
  delivered: { label: 'Delivered', className: 'border-accent/35 bg-accent/10 text-accent' },
...
function DelistingChip({ task, onComplete, disabled }: { task: DelistingTask | undefined; onComplete: () => void; disabled: boolean }) {
  ...
  return <div className={`flex items-center justify-between gap-2 rounded-lg border px-2.5 py-2 ${complete ? 'border-accent/35 bg-accent/10 text-accent' : task.status === 'failed' ? 'border-destructive/35 bg-destructive/10 text-destructive' : 'border-amber-400/30 bg-amber-400/10 text-amber-200'}`}><span className="font-mono text-[0.62rem] uppercase">{task.marketplace[0]} · {complete ? 'delisted' : task.status.replace('_', ' ')}</span>{!complete && <button type="button" onClick={onComplete} disabled={disabled} className="rounded border border-current/30 px-1.5 py-0.5 text-[0.58rem] font-bold hover:bg-black/10 disabled:opacity-50">Confirm</button>}</div>;
}
...
export default function Orders() {
  ...
  if (query.isLoading) return <div className="h-64 animate-pulse rounded-2xl border border-border bg-white/5" />;
  ...
  return <div className="space-y-6">
    ...
    {orders.length === 0 ? <section className="cx-panel rounded-2xl p-12 text-center"><PackageCheck size={28} className="mx-auto text-muted-foreground" /><p className="mt-4 font-semibold text-foreground">No sales recorded yet.</p><p className="mt-2 text-sm text-muted-foreground">When you record a sale from Listing Studio, it will appear here with delisting and fulfillment actions.</p></section> : <section className="space-y-5">{orders.map((order) => {
      ...
      return <article key={order.id} className="cx-panel overflow-hidden rounded-2xl"><div className="flex flex-col gap-4 border-b border-border/70 bg-background/25 p-5 sm:p-6 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2.5 py-1 font-mono text-[0.6rem] uppercase tracking-wider ${status.className}`}>{status.label}</span><span className="rounded-full border border-primary/30 bg-primary/5 px-2.5 py-1 font-mono text-[0.6rem] uppercase tracking-wider text-primary">Sold on {order.marketplace}</span></div><h2 className="mt-3 text-xl font-semibold text-foreground">{order.itemTitle || 'Untitled item'}</h2><p className="mt-2 text-sm text-muted-foreground">{new Date(order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} · Buyer: <span className="font-semibold text-foreground">{order.buyerName || 'Not recorded'}</span>{order.trackingNumber ? <> · Tracking: <span className="font-mono text-xs text-foreground">{order.trackingNumber}</span></> : null}</p></div><div className="grid grid-cols-3 gap-5 text-right"><div><p className="font-mono text-sm text-accent">{money(order.salePrice)}</p><p className="mt-1 text-[0.58rem] uppercase tracking-wider text-muted-foreground">Sold for</p></div><div><p className="font-mono text-sm text-muted-foreground">{money(order.fees + order.shippingCost)}</p><p className="mt-1 text-[0.58rem] uppercase tracking-wider text-muted-foreground">Fees + ship</p></div><div><p className={`font-mono text-sm ${order.profit >= 0 ? 'text-primary' : 'text-destructive'}`}>{money(order.profit)}</p><p className="mt-1 text-[0.58rem] uppercase tracking-wider text-muted-foreground">Profit</p></div></div></div><div className="grid gap-0 xl:grid-cols-[0.95fr_1.05fr]"><section className="border-b border-border/70 p-5 sm:p-6 xl:border-b-0 xl:border-r"><div className="flex items-start justify-between gap-3"><div><p className="cx-eyebrow">Remaining listing removal</p><h3 className="mt-2 text-lg font-semibold">Delist the other platforms</h3><p className="mt-2 text-sm leading-5 text-muted-foreground">This sale happened on <span className="font-semibold text-foreground capitalize">{order.marketplace}</span>. Confirm removal only after you have actually delisted the remaining listing.</p></div><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${delisting.length > 0 && completedDelists === delisting.length ? 'border-accent/35 bg-accent/10 text-accent' : 'border-amber-400/30 bg-amber-400/10 text-amber-200'}`}>{completedDelists}/{delisting.length} delisted</span></div><div className="mt-5 grid gap-2">{delisting.length === 0 ? <div className="rounded-lg border border-border bg-background/35 p-3 text-sm text-muted-foreground">No remaining listing records need delisting for this sale.</div> : MARKETPLACES.filter((platform) => platform !== order.marketplace).map((platform) => <DelistingChip key={platform} task={delisting.find((task) => task.marketplace === platform)} onComplete={() => { const task = delisting.find((entry) => entry.marketplace === platform); if (task) void completeDelisting(task.id); }} disabled={isMutating} />)}</div>{delisting.length > 0 && completedDelists === delisting.length && <div className="mt-4 flex items-center gap-2 rounded-lg border border-accent/30 bg-accent/5 px-3 py-2 text-xs font-semibold text-accent"><Check size={14} />All remaining marketplace listings are confirmed delisted.</div>}</section><section className="p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><p className="cx-eyebrow">Pull · print · pack · ship</p><h3 className="mt-2 text-lg font-semibold">Fulfillment checklist</h3><p className="mt-2 text-sm leading-5 text-muted-foreground">{shipping ? `${completedSteps}/${totalSteps} steps completed for this sale.` : 'This order has no active fulfillment checklist.'}</p></div><Link href="/shipping" className="inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline">Full queue <ChevronRight size={14} /></Link></div>{shipping ? <div className="mt-5 grid gap-1.5">{shipping.steps.map((step) => <button key={step.key} type="button" disabled={isMutating} onClick={() => void toggleShippingStep(shipping, step.key)} className="flex items-center gap-3 rounded-lg px-2 py-2 text-left transition hover:bg-white/5 disabled:opacity-50"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${step.completed ? 'border-accent bg-accent text-accent-foreground' : 'border-muted-foreground/50 text-muted-foreground'}`}>{step.completed ? <Check size={12} /> : <ClipboardList size={11} />}</span><span className={`text-sm ${step.completed ? 'text-muted-foreground line-through' : 'text-foreground'}`}>{step.label}</span></button>)}</div> : <div className="mt-5 rounded-lg border border-dashed border-border bg-background/30 p-4 text-sm text-muted-foreground">This sale will appear in fulfillment when its order status is awaiting shipment.</div>}</section></div></article>;
    })}</section>}
    <section className="rounded-xl border border-border bg-background/30 p-4 text-sm text-muted-foreground"><div className="flex gap-3"><MapPin size={17} className="mt-0.5 shrink-0 text-primary" /><p><span className="font-semibold text-foreground">Workflow guardrail:</span> this page never silently claims a platform listing is removed. A delisting chip changes only after you confirm the removal. The actual shipment steps are recorded in the fulfillment checklist.</p></div></section>
  </div>;
}
