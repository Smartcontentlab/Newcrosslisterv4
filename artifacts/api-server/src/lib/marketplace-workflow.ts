...
export async function enqueueDelistingTasksForSale(input: {
  ...
  const [listings, drafts, existing] = await Promise.all([
    db.select().from(listingsTable).where(and(eq(listingsTable.itemId, input.itemId), eq(listingsTable.status, "active"))),
    db.select().from(marketplaceDraftsTable).where(eq(marketplaceDraftsTable.itemId, input.itemId)),
    db.select().from(delistingTasksTable).where(eq(delistingTasksTable.itemId, input.itemId)),
  ]);
  ...
  const openTasks = new Set(
    existing.filter((task) => ["pending", "in_progress"].includes(task.status)).map((task) => task.marketplace)
  );
  ...
  if (tasksToInsert.length > 0) {
    await db.insert(delistingTasksTable).values(tasksToInsert);
  }
  ...
  if (draftMarketplaces.length > 0) {
    await db
      .update(marketplaceDraftsTable)
      .set({ status: "delisting" })
      .where(and(eq(marketplaceDraftsTable.itemId, input.itemId), inArray(marketplaceDraftsTable.marketplace, draftMarketplaces)));
  }
  await db
    .update(marketplaceDraftsTable)
    .set({ status: "sold", soldAt: new Date() })
    .where(and(eq(marketplaceDraftsTable.itemId, input.itemId), eq(marketplaceDraftsTable.marketplace, input.soldMarketplace)));
